...
def aug_batch(img_ref, gt_ref, img_search, gt_search, no_crop=False):
    ...
    if bb[2] != 0 and bb[3] != 0:
        for i in range(10):
            ...
            mask_ref_gt  = np.expand_dims(gt_ref_map.get_arr_int().astype('uint8'), 2)
            box_ref = seq_deform.augment_segmentation_maps([gt_ref_map])[0].get_arr_int().astype(float)

            #scale_p = random.uniform(0.8, 1.2)
            bb = scale_box(box_ref)
            #bb = cv2.boundingRect(template_mask.astype('uint8'))
            if bb[2] >= 15 and bb[3] >= 15:
                img_ref = seq_ref_det.augment_image(img_ref_o)
                box_ref = np.zeros([*box_ref.shape[:2], 1])
                box_ref[bb[1]:bb[1]+bb[3]+1, bb[0]:bb[0]+bb[2]+1, 0] = 1
                break
        else:
            img_ref = cv2.resize(img_ref_o, (dim, dim))
            mask_ref_gt = np.expand_dims(cv2.resize(gt_ref_o, (dim, dim)), 2)
            box_ref = mask_ref_gt.copy()
            bb = scale_box(box_ref)
            if bb[2] >= 15 and bb[3] >= 15:
                box_ref = np.zeros([*box_ref.shape[:2], 1])
                box_ref[bb[1]:bb[1]+bb[3]+1, bb[0]:bb[0]+bb[2]+1, 0] = 1
        ...
        mask_ref_gt = np.zeros([dim, dim, 1])
        box_ref = mask_ref_gt.copy()
    ...
    return img_search, p_mask, img_prev, img_ref, box_ref, gt_search, mask_ref_gt
