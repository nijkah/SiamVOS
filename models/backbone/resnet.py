...
class ResNet(nn.Module):
    ...
    def _load_imagenet_1k(self):
        pretrained_dict = model_zoo.load_url(self.pretrained)
        model_dict = {}
        ...
        for k, v in pretrained_dict.items():
            ...
            elif self.in_channel == 6 and name_parts[0] == 'conv1':
                 model_dict[k] = torch.cat((v, v), 1)
...
def build_resnet(in_channel, layers='101', ms=False, pretrained=True):
    models = {
    ...
    block, layers_info= models[str(layers)]
    if pretrained:
        pretrained = model_urls[str(layers)]

    if ms:
        model = ResNet_ms(block, layers_info, in_channel, pretrained)
    else:
        model = ResNet(block, layers_info, in_channel, pretrained)
